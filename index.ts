...
async function claimTokenFaucet(tokenAddress: string, walletAddress: string, authToken: string): Promise<string | null | boolean> {
  ...
  const faucetStatus = await checkFaucetStatus(walletAddress, authToken);
  if (faucetStatus && !faucetStatus.is_able_to_faucet) {
    console.log(
      chalk.yellow(
        `${getEmoji('hourglass')} ${tokenName} Faucet on cooldown for ${walletAddress} until ${new Date(
          faucetStatus.available_timestamp * 1000
        ).toUTCString()}`
      )
    );
    return null;
  ...
  while (attempt < retries) {
    try {
      console.log(
        chalk.blue(
          `${getEmoji('coin')} Attempting to claim ${tokenName} faucet for ${walletAddress} (Attempt ${attempt + 1}/${retries})`
        )
      );
      ...
      console.log(chalk.blue(`${getEmoji('mag')} Faucet API Response: ${JSON.stringify(response.data)}`));
      ...
      if (response.data && response.data.status === 200 && response.data.data && response.data.data.txHash) {
        ...
        console.log(
          chalk.yellow(`${getEmoji('warning')} ${tokenName} Faucet for ${walletAddress}: No transaction hash in response`)
        );
        attempt++;
        if (attempt >= retries) {
          console.error(
            chalk.red(`${getEmoji('x')} ${tokenName} Faucet failed for ${walletAddress} after ${retries} attempts`)
          );
          return null;
        }
        const delayTime = Math.min(10000 * Math.pow(2, attempt), 30000);
        console.log(chalk.yellow(`${getEmoji('hourglass')} Waiting ${delayTime / 1000} seconds before retrying faucet...`));
      ...
      console.error(
        chalk.red(
          `${getEmoji('x')} ${tokenName} Faucet Error for ${walletAddress} (Attempt ${attempt}/${retries}): ${statusCode} - ${errorMsg}`
        )
      );
      console.log(chalk.yellow(`${getEmoji('mag')} Error Details: ${JSON.stringify(error.response?.data || error)}`));
      if (attempt >= retries) {
        console.error(
          chalk.red(`${getEmoji('x')} ${tokenName} Faucet failed for ${walletAddress} after ${retries} attempts`)
        );
...
async function checkBalance(tokenAddress: string | null, walletAddress: string): Promise<{ formatted: string; raw: ethers.BigNumber }> {
  const maxRetries = 3;
  let attempt = 0;
  ...
  // Handle PHRS native token
  if (!tokenAddress || tokenAddress.toLowerCase() === 'phrs' || tokenAddress === ethers.constants.AddressZero) {
    while (attempt < maxRetries) {
      try {
        const balance: ethers.BigNumber = await provider.getBalance(walletAddress);
        const formatted: string = ethers.utils.formatEther(balance); // PHRS uses 18 decimals
        console.log(chalk.blue(`${getEmoji('bar_chart')} PHRS Balance for ${walletAddress}: ${formatted}`));
        return { formatted, raw: balance };
      } catch (error: any) {
        console.error(
          chalk.red(
            `${getEmoji('x')} PHRS Balance Check Error for ${walletAddress} (Attempt ${attempt + 1}/${maxRetries}): ${error.message}`
          )
        );
        attempt++;
        if (attempt < maxRetries) {
          const delayTime = Math.min(2000 * Math.pow(2, attempt), 10000);
          console.log(chalk.yellow(`${getEmoji('hourglass')} Retrying balance check in ${delayTime / 1000} seconds...`));
          await new Promise((resolve) => setTimeout(resolve, delayTime));
        }
      }
    }
    console.error(chalk.red(`${getEmoji('x')} Failed to check PHRS balance for ${walletAddress} after ${maxRetries} attempts`));
    return { formatted: '0', raw: ethers.BigNumber.from(0) };
  }

  // Handle ERC-20 tokens
  const tokenConfig = Object.values(TOKENS).find((t) => t.address.toLowerCase() === tokenAddress.toLowerCase());
  if (!tokenConfig) {
    console.error(chalk.red(`${getEmoji('x')} Invalid token address: ${tokenAddress}`));
    return { formatted: '0', raw: ethers.BigNumber.from(0) };
  }
  ...
  while (attempt < maxRetries) {
    try {
      ...
      console.log(chalk.blue(`${getEmoji('bar_chart')} ${tokenConfig.name} Balance for ${walletAddress}: ${formatted}`));
      return { formatted, raw: balance };
    } catch (error: any) {
      console.error(
        chalk.red(
          `${getEmoji('x')} ${tokenConfig.name} Balance Check Error for ${walletAddress} (Attempt ${attempt + 1}/${maxRetries}): ${error.message}`
        )
      );
      attempt++;
      if (attempt < maxRetries) {
  ...
  console.error(chalk.red(`${getEmoji('x')} Failed to check ${tokenConfig.name} balance for ${walletAddress} after ${maxRetries} attempts`));
  return { formatted: '0', raw: ethers.BigNumber.from(0) };
}

// Ensure PHRS balance
async function ensurePhrsBalance(walletAddress: string, authToken: string): Promise<boolean> {
  const minPhrs = ethers.utils.parseEther('0.1'); // Minimum PHRS required
  const balanceInfo = await checkBalance(null, walletAddress); // Use null for PHRS
  console.log(chalk.blue(`${getEmoji('bar_chart')} PHRS Balance for ${walletAddress}: ${balanceInfo.formatted} PHRS`));

  if (balanceInfo.raw.gte(minPhrs)) {
    console.log(chalk.green(`${getEmoji('white_check_mark')} Sufficient PHRS balance for ${walletAddress}: ${balanceInfo.formatted} PHRS`));
    return true;
  }

  console.warn(chalk.yellow(`${getEmoji('warning')} Insufficient PHRS balance for ${walletAddress}: ${balanceInfo.formatted} PHRS, attempting faucet...`));
  const faucetResult = await claimDailyFaucet(walletAddress, authToken);
  if (faucetResult) {
    const newBalanceInfo = await checkBalance(null, walletAddress);
    if (newBalanceInfo.raw.gte(minPhrs)) {
      console.log(chalk.green(`${getEmoji('white_check_mark')} PHRS balance sufficient after faucet for ${walletAddress}: ${newBalanceInfo.formatted} PHRS`));
      return true;
    }
    console.error(chalk.red(`${getEmoji('x')} PHRS balance still insufficient after faucet for ${walletAddress}: ${newBalanceInfo.formatted} PHRS`));
  } else {
    console.error(chalk.red(`${getEmoji('x')} PHRS faucet claim failed for ${walletAddress}`));
  }
  return false;
}
...
async function addLiquidity(walletAddress: string, authToken: string, times: number): Promise<number> {
  ...
  for (let i = 0; i < times; i++) {
    try {
      console.log(
        chalk.cyan(
          `${getEmoji('droplet')} Adding liquidity ${i + 1}/${times} for ${walletAddress}: ${amount0} WPHRS / ${amount1} USDC`
        )
      );
      ...
      const timeoutPromise = new Promise<undefined>((resolve) => setTimeout(() => resolve(undefined), 60000));
      ...
      // Verify the liquidity addition
      console.log(chalk.blue(`${getEmoji('hourglass')} Verifying liquidity task (ID 103) for ${walletAddress}...`));
      ...
      if (i < times - 1) {
        ...
        console.log(
          chalk.blue(`${getEmoji('hourglass')} Waiting ${delayTime / 1000} seconds before next liquidity addition for ${walletAddress}`)
        );
        await new Promise((resolve) => setTimeout(resolve, delayTime));
      }
    } catch (error: any) {
  ...
  console.log(
    chalk.green(
      `${getEmoji('chart_with_upwards_trend')} Liquidity Summary for ${walletAddress}: ${successCount}/${times} successful additions`
    )
  );
  return successCount;
}
...
async function swapTokens(
  ...
  const provider = new ethers.providers.JsonRpcProvider('https://testnet.dplabs-internal.com');
  const routerContract = new ethers.Contract(ROUTER_ADDRESS, ROUTER_ABI, wallet.connect(provider));
  ...
  const pair = {
    from: TOKENS[Object.keys(TOKENS).find((k: string) => TOKENS[k].address.toLowerCase() === tokenIn.toLowerCase())!]?.name || 'Unknown',
    to: TOKENS[Object.keys(TOKENS).find((k: string) => TOKENS[k].address.toLowerCase() === tokenOut.toLowerCase())!]?.name || 'Unknown',
  };

  const decimals: number = TOKENS[
    Object.keys(TOKENS).find((k: string) => TOKENS[k].address.toLowerCase() === tokenIn.toLowerCase())!
  ]?.decimals || 6;
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        if (!balanceApprovalSuccess) {
          console.error(
            chalk.red(
              `${getEmoji('warning')} Swap ${i + 1} failed for ${walletAddress}: Insufficient balance or approval after retries`
            )
          );
          break;
        }
        ...
        if (!multicallData || multicallData.length === 0 || multicallData.some((data) => !data || data === '0x')) {
          console.error(
            chalk.red(`${getEmoji('warning')} Invalid or empty multicall data for ${pair.from} -> ${pair.to} for ${walletAddress}`)
          );
          break;
        }
        ...
        try {
          ...
          console.error(
            chalk.red(`${getEmoji('x')} Gas estimation failed for swap ${i + 1} for ${walletAddress}: ${error.message}`)
          );
          attempt++;
          if (attempt >= maxAttempts) {
          ...
          const delayTime: number = Math.min(2000 * Math.pow(2, attempt), 15000);
        ...
        const timeoutPromise = new Promise<undefined>((resolve) => setTimeout(() => resolve(undefined), 120000));
        ...
        while (receiptAttempts < maxReceiptAttempts) {
          try {
            ...
            receipt = await Promise.race([txResponse.wait(), timeoutPromise]);
            if (!receipt) {
              console.error(
                chalk.yellow(
                  `${getEmoji('warning')} Transaction receipt not found for ${tx.hash}, retrying (${receiptAttempts + 1}/${maxReceiptAttempts})...`
                )
              );
              receiptAttempts++;
            ...
            console.error(
              chalk.red(
                `${getEmoji('x')} Swap ${i + 1} Error for ${walletAddress} (Receipt Attempt ${receiptAttempts + 1}/${maxReceiptAttempts}): ${error.message}`
              )
            );
            receiptAttempts++;
            if (receiptAttempts >= maxReceiptAttempts) {
              console.error(
                chalk.red(`${getEmoji('x')} Swap ${i + 1} failed for ${walletAddress} after ${maxReceiptAttempts} receipt attempts`)
              );
              break;
            }
            const delayTime = Math.min(2000 * Math.pow(2, receiptAttempts), 15000);
            console.log(chalk.yellow(`${getEmoji('hourglass')} Retrying receipt in ${delayTime / 1000} seconds...`));
            await new Promise((resolve) => setTimeout(resolve, delayTime));
          }
        }
        ...
        console.log(chalk.blue(`${getEmoji('hourglass')} Verifying swap task (ID 103) for ${walletAddress}...`));
        const verificationSuccess = await verifyTask(walletAddress, authToken, '103', receipt.transactionHash);
        if (verificationSuccess) {
          console.log(
            chalk.green(
        ...
        attempt++;
        console.error(
          chalk.red(`${getEmoji('x')} Swap ${i + 1} Error for ${walletAddress} (Attempt ${attempt}/${maxAttempts}): ${error.message}`)
        );
...
async function runDailyTasks(featureNumber: number, params: SwapParams, wallet: WalletConfig): Promise<FaucetStatus | null> {
  ...
  if (featureNumber === 1 || featureNumber === 2 || featureNumber === 4 || featureNumber === 5) {
    if (params.tokenIn) {
      await claimTokenFaucet(params.tokenIn, wallet.address, wallet.authToken);
    }
    if (featureNumber === 5) {
      await claimTokenFaucet(TOKENS.WPHRS.address, wallet.address, wallet.authToken);
    }
  }
